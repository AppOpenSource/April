...
public class AlbumHelper {
	...
	private HashMap<String, ImgBucket> mBucketList = new HashMap<String, ImgBucket>();
	...
	private void getThumbnail() {
		String[] projection = { Thumbnails._ID, Thumbnails.IMAGE_ID,
				Thumbnails.DATA };
		Cursor cursor = mResolver.query(Thumbnails.EXTERNAL_CONTENT_URI, projection,
				null, null, null);
	...
	private void getThumbnailColumnData(Cursor cur) {
		if (cur.moveToFirst()) {
			...
			do {
				...
				// LogManager.i(TAG, _id + " image_id:" + image_id + " path:"
				// + image_path + "---");
	...
	public void getAlbum() {
		...
		Cursor cursor = mResolver.query(Albums.EXTERNAL_CONTENT_URI, projection, null,
				null, null);
	...
	public void buildImagesBucketList() {
		...
		if (cur.moveToFirst()) {
			...
			do {
				...
				if (bucket == null) {
					...
					bucket.imageList = new ArrayList<ImgItem>();
					bucket.bucketName = bucketName;
				}
				bucket.count++;
				ImgItem imageItem = new ImgItem();
		...
		Iterator<Entry<String, ImgBucket>> itr = mBucketList.entrySet()
				.iterator();
		while (itr.hasNext()) {
	 ...
	 * 得到图片集
	 */
	public List<ImgBucket> getImagesBucketList(boolean refresh) {
		if (refresh || (!refresh && !mHasBuildImagesBucketList)) {
			buildImagesBucketList();
		}
		List<ImgBucket> tmpList = new ArrayList<ImgBucket>();
		Iterator<Entry<String, ImgBucket>> itr = mBucketList.entrySet()
				.iterator();
		while (itr.hasNext()) {
			Entry<String, ImgBucket> entry = (Entry<String, ImgBucket>) itr
