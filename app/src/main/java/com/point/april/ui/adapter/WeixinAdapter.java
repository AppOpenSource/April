...
import com.point.april.ui.activity.news.WeiXinActivity;
import com.point.april.util.DBUtils;
import com.point.april.util.SharePreferenceUtil;
import com.point.base.control.ScreenUtil;
...
public class WeixinAdapter extends RecyclerView.Adapter<WeixinAdapter.WeixinViewHolder> {
    ...
    public void onBindViewHolder(final WeixinViewHolder holder, int position) {
        ...
        if (DBUtils.getDB(mContext).isRead(GlobalConstant.WEIXIN, weixinNews.getUrl(), 1))
        ...
        holder.btnWeixin.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                final boolean isRead = DBUtils.getDB(mContext).isRead(GlobalConstant.WEIXIN, weixinNews.getUrl(), 1);
                if (!isRead)
                    ...
                    public boolean onMenuItemClick(MenuItem item) {
                        switch (item.getItemId()) {
                            case R.id.pop_unread:
                                if (isRead) {
                                    DBUtils.getDB(mContext).insertHasRead(GlobalConstant.WEIXIN, weixinNews.getUrl(), 0);
                                    holder.tvTitle.setTextColor(Color.BLACK);
                                } else {
                                    DBUtils.getDB(mContext).insertHasRead(GlobalConstant.WEIXIN, weixinNews.getUrl(), 1);
                                    holder.tvTitle.setTextColor(Color.GRAY);
                                }
                                break;
                            case R.id.pop_share:
      ...
      //  @BindView(R.id.tv_description)
       ...
       // @BindView(R.id.btn_weixin)
        ...
        public WeixinViewHolder(View itemView) {
            super(itemView);
            ivWeixin = (ImageView) itemView.findViewById(R.id.iv_weixin);
            tvTitle = (TextView) itemView.findViewById(R.id.tv_title);
            tvTime = (TextView) itemView.findViewById(R.id.tv_time);
            tvDescription = (TextView) itemView.findViewById(R.id.tv_description);
            cvMain = (CardView) itemView.findViewById(R.id.cv_main);
            btnWeixin = (Button) itemView.findViewById(R.id.btn_weixin);
